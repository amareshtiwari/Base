...
class Codesbug_Base_Model_Observer extends Varien_Object
{
    ...
    public function baseRule($condition_data, $orderData)
    {
        ...
        foreach ($items as $item) {
            ...
            foreach ($condition_data['conditions'] as $cond) {
                 ...
                 */
                $condition_check_address = strcmp(trim($condition_type), 'base/rule_condition_address');
                $condition_check_product = strcmp(trim($condition_type), 'base/rule_condition_product');
                $condition_check_customer = strcmp(trim($condition_type), 'base/rule_condition_customer');
                $condition_check_cumbine = strcmp(trim($condition_type), 'base/rule_condition_combine');
                $condition_check_subselect = strcmp(trim($condition_type), 'salesrule/rule_condition_product_subselect');
                $condition_check_salesrule_cumbine = strcmp(trim($condition_type), 'salesrule/rule_condition_product_combine');
                $condition_check_salesrule_product = strcmp(trim($condition_type), 'salesrule/rule_condition_product');
                $condition_check_salesrule_product_found = strcmp(trim($condition_type), 'salesrule/rule_condition_product_found');
                $condition_check_salesrule_product_attr_assigned = strcmp(trim($condition_type), 'salesrule/rule_condition_product_attribute_assigned');
                ...
                if ($condition_check_address == 0) {
                    if ($attribute == 'base_subtotal') {
                        if (Mage::getSingleton('checkout/cart')->getQuote()) {
                            $type_attribute = Mage::getSingleton('checkout/cart')->getQuote()->getBaseSubtotal();
                        }
                        ...
                        if (Mage::getSingleton('checkout/cart')->getQuote()) {
                            ...
                            if (!empty($paymentMethod)) {
                                $type_attribute = Mage::getSingleton('checkout/session')->getQuote()->getPayment()->getMethodInstance()->getCode();
                            }
                        }
                    } else {
